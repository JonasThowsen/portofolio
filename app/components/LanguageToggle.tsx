...
const LanguageToggle = () => {
  ...
  return (
    <div className="flex items-center">
      ...
      <button
        className="w-14 h-7 flex items-center bg-gray-300 rounded-full p-1 forst"
        onClick={toggleLanguage}
        aria-label={`Switch to ${language === 'en' ? 'Norwegian' : 'English'}`}
      >
        <div
          className={`bg-white w-5 h-5 rounded-full shadow-md transform sist ${
            language === 'no' ? 'translate-x-7' : 'translate-x-0'
          }`}
        />
      </button>
